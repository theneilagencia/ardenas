/**
 * Arden.AS — API v1 · erros (ARDEN-FE-003).
 *
 * Resposta de erro ÚNICA e tipada. Nunca expõe stack trace nem detalhes internos
 * de banco. `correlationId` está sempre presente. Os códigos são estáveis e
 * mapeiam para status HTTP documentados (ver API_V1_ERROR_CATALOG.md).
 */

import { z } from 'zod';
import { correlationId } from './identifiers';

export const apiErrorCode = z.enum([
  'VALIDATION_ERROR',
  'UNAUTHENTICATED',
  'SESSION_EXPIRED',
  'FORBIDDEN',
  'ORGANIZATION_REQUIRED',
  'ORGANIZATION_SUSPENDED',
  'MEMBERSHIP_REQUIRED',
  'MEMBERSHIP_SUSPENDED',
  'RESOURCE_NOT_FOUND',
  'VERSION_CONFLICT',
  'RESOURCE_CONFLICT',
  'ALREADY_PUBLISHED',
  'INVALID_STATE_TRANSITION',
  'IDEMPOTENCY_CONFLICT',
  // Governança, aprovações e autorizações de ação (ARDEN-BE-004).
  'APPROVAL_REQUIRED',
  'APPROVAL_NOT_PENDING',
  'APPROVAL_NOT_ELIGIBLE',
  'SELF_APPROVAL_FORBIDDEN',
  'APPROVAL_EXPIRED',
  'APPROVAL_INVALIDATED',
  'APPROVAL_ALREADY_DECIDED',
  'QUORUM_NOT_REACHED',
  'POLICY_CONFLICT',
  'POLICY_NOT_ACTIVE',
  'ACTION_DENIED',
  'ACTION_NOT_DECLARED',
  'AUTHORIZATION_EXPIRED',
  'AUTHORIZATION_INVALIDATED',
  'AUTHORIZATION_PAYLOAD_MISMATCH',
  // Motor de execução (ARDEN-BE-005).
  'EXECUTION_NOT_ALLOWED',
  'EXECUTION_ALREADY_STARTED',
  'EXECUTION_NOT_PAUSABLE',
  'EXECUTION_NOT_RESUMABLE',
  'EXECUTION_NOT_CANCELLABLE',
  'EXECUTION_NOT_RETRYABLE',
  'EXECUTION_TERMINAL',
  'EXECUTION_TIMED_OUT',
  'EXECUTION_RETRY_EXHAUSTED',
  'STEP_EXECUTOR_NOT_AVAILABLE',
  'STEP_INPUT_INVALID',
  'STEP_FAILED',
  'STEP_TIMED_OUT',
  'AUTHORIZATION_REQUIRED',
  'AUTHORIZATION_ALREADY_USED',
  'JOB_LEASE_CONFLICT',
  'JOB_NOT_RECOVERABLE',
  // Conectores, credenciais, ferramentas e webhooks (ARDEN-BE-006).
  'CONNECTOR_NOT_AVAILABLE',
  'CONNECTOR_DEPRECATED',
  'CONNECTION_NOT_ACTIVE',
  'CONNECTION_SUSPENDED',
  'CONNECTION_REVOKED',
  'CONNECTION_TEST_FAILED',
  'CREDENTIAL_REQUIRED',
  'CREDENTIAL_INVALID',
  'CREDENTIAL_REVOKED',
  'CREDENTIAL_RESOLUTION_FAILED',
  'CREDENTIAL_ROTATION_CONFLICT',
  'TOOL_NOT_AVAILABLE',
  'TOOL_BINDING_NOT_FOUND',
  'TOOL_INPUT_INVALID',
  'TOOL_OUTPUT_INVALID',
  'TOOL_EXECUTION_DENIED',
  'NETWORK_POLICY_DENIED',
  'HOST_NOT_ALLOWED',
  'PROTOCOL_NOT_ALLOWED',
  'PRIVATE_NETWORK_DENIED',
  'SSRF_BLOCKED',
  'REDIRECT_DENIED',
  'REQUEST_TOO_LARGE',
  'RESPONSE_TOO_LARGE',
  'EXTERNAL_TIMEOUT',
  'EXTERNAL_RATE_LIMITED',
  'EXTERNAL_PROVIDER_ERROR',
  'EXTERNAL_RESULT_UNKNOWN',
  'WEBHOOK_SIGNATURE_INVALID',
  'WEBHOOK_TIMESTAMP_INVALID',
  'WEBHOOK_REPLAYED',
  'WEBHOOK_EVENT_NOT_ALLOWED',
  'WEBHOOK_ENDPOINT_REVOKED',
  'WEBHOOK_ENDPOINT_SUSPENDED',
  'WEBHOOK_TRIGGER_DENIED',
  'WEBHOOK_DELIVERY_CONFLICT',
  // Runtime de agentes e modelos (ARDEN-BE-007).
  'AGENT_NOT_FOUND',
  'AGENT_NOT_ACTIVE',
  'AGENT_SUSPENDED',
  'AGENT_REVOKED',
  'AGENT_VERSION_NOT_FOUND',
  'AGENT_VERSION_NOT_PUBLISHED',
  'AGENT_VERSION_RETIRED',
  'MODEL_PROVIDER_NOT_AVAILABLE',
  'MODEL_PROVIDER_DISABLED',
  'MODEL_CONFIGURATION_NOT_FOUND',
  'MODEL_CONFIGURATION_NOT_ACTIVE',
  'AGENT_CONTEXT_TOO_LARGE',
  'AGENT_INPUT_INVALID',
  'AGENT_OUTPUT_INVALID',
  'AGENT_OUTPUT_REPAIR_EXHAUSTED',
  'AGENT_TOOL_NOT_ALLOWED',
  'AGENT_TOOL_CALL_INVALID',
  'AGENT_TOOL_CALL_LIMIT_EXCEEDED',
  'AGENT_TOOL_REQUIRES_APPROVAL',
  'AGENT_TURN_LIMIT_EXCEEDED',
  'AGENT_TOKEN_LIMIT_EXCEEDED',
  'AGENT_COST_LIMIT_EXCEEDED',
  'AGENT_TIMEOUT',
  'MODEL_RATE_LIMITED',
  'MODEL_PROVIDER_ERROR',
  'MODEL_RESULT_UNKNOWN',
  'MODEL_CONTENT_FILTERED',
  'AGENT_EVALUATION_FAILED',
  'AGENT_PROMPT_INJECTION_DETECTED',
  'AGENT_CONTEXT_SOURCE_NOT_ALLOWED',
  'AGENT_CONTEXT_SOURCE_NOT_FOUND',
  'AGENT_CONTEXT_SOURCE_INVALID',
  'AGENT_CONTEXT_BUDGET_EXCEEDED',
  'AGENT_RESULT_NOT_FOUND',
  'AGENT_GOVERNANCE_BLOCKED',
  'AGENT_USAGE_INVALID',
  'RATE_LIMITED',
  'DEPENDENCY_UNAVAILABLE',
  'INTERNAL_ERROR',
]);
export type ApiErrorCode = z.infer<typeof apiErrorCode>;

export const fieldError = z.object({
  field: z.string(),
  code: z.string(),
  message: z.string(),
});
export type FieldError = z.infer<typeof fieldError>;

export const apiErrorBody = z.object({
  code: apiErrorCode,
  message: z.string(),
  correlationId,
  /** Detalhes seguros (ex.: revisão esperada/atual em conflito). Nunca dados internos. */
  details: z.record(z.unknown()).optional(),
  fieldErrors: z.array(fieldError).optional(),
});
export type ApiErrorBody = z.infer<typeof apiErrorBody>;

/** Envelope de erro. */
export const apiErrorResponse = z.object({ error: apiErrorBody });
export type ApiErrorResponse = z.infer<typeof apiErrorResponse>;

/** Detalhe padronizado de conflito de concorrência (VERSION_CONFLICT). */
export const versionConflictDetails = z.object({
  resourceType: z.string(),
  resourceId: z.string(),
  expectedRevision: z.number().int(),
  currentRevision: z.number().int(),
});
export type VersionConflictDetails = z.infer<typeof versionConflictDetails>;

/** Mapa canônico código → status HTTP (documentado em API_V1_ERROR_CATALOG.md). */
export const ERROR_HTTP_STATUS: Record<ApiErrorCode, number> = {
  VALIDATION_ERROR: 422,
  UNAUTHENTICATED: 401,
  SESSION_EXPIRED: 401,
  FORBIDDEN: 403,
  ORGANIZATION_REQUIRED: 403,
  ORGANIZATION_SUSPENDED: 403,
  MEMBERSHIP_REQUIRED: 403,
  MEMBERSHIP_SUSPENDED: 403,
  RESOURCE_NOT_FOUND: 404,
  VERSION_CONFLICT: 409,
  RESOURCE_CONFLICT: 409,
  ALREADY_PUBLISHED: 409,
  INVALID_STATE_TRANSITION: 409,
  IDEMPOTENCY_CONFLICT: 409,
  APPROVAL_REQUIRED: 403,
  APPROVAL_NOT_PENDING: 409,
  APPROVAL_NOT_ELIGIBLE: 403,
  SELF_APPROVAL_FORBIDDEN: 403,
  APPROVAL_EXPIRED: 409,
  APPROVAL_INVALIDATED: 409,
  APPROVAL_ALREADY_DECIDED: 409,
  QUORUM_NOT_REACHED: 409,
  POLICY_CONFLICT: 409,
  POLICY_NOT_ACTIVE: 409,
  ACTION_DENIED: 403,
  ACTION_NOT_DECLARED: 409,
  AUTHORIZATION_EXPIRED: 409,
  AUTHORIZATION_INVALIDATED: 409,
  AUTHORIZATION_PAYLOAD_MISMATCH: 409,
  EXECUTION_NOT_ALLOWED: 403,
  EXECUTION_ALREADY_STARTED: 409,
  EXECUTION_NOT_PAUSABLE: 409,
  EXECUTION_NOT_RESUMABLE: 409,
  EXECUTION_NOT_CANCELLABLE: 409,
  EXECUTION_NOT_RETRYABLE: 409,
  EXECUTION_TERMINAL: 409,
  EXECUTION_TIMED_OUT: 409,
  EXECUTION_RETRY_EXHAUSTED: 409,
  STEP_EXECUTOR_NOT_AVAILABLE: 409,
  STEP_INPUT_INVALID: 422,
  STEP_FAILED: 409,
  STEP_TIMED_OUT: 409,
  AUTHORIZATION_REQUIRED: 403,
  AUTHORIZATION_ALREADY_USED: 409,
  JOB_LEASE_CONFLICT: 409,
  JOB_NOT_RECOVERABLE: 409,
  // Conectores, credenciais, ferramentas e webhooks (ARDEN-BE-006).
  CONNECTOR_NOT_AVAILABLE: 404,
  CONNECTOR_DEPRECATED: 409,
  CONNECTION_NOT_ACTIVE: 409,
  CONNECTION_SUSPENDED: 409,
  CONNECTION_REVOKED: 409,
  CONNECTION_TEST_FAILED: 502,
  CREDENTIAL_REQUIRED: 409,
  CREDENTIAL_INVALID: 422,
  CREDENTIAL_REVOKED: 409,
  CREDENTIAL_RESOLUTION_FAILED: 500,
  CREDENTIAL_ROTATION_CONFLICT: 409,
  TOOL_NOT_AVAILABLE: 404,
  TOOL_BINDING_NOT_FOUND: 404,
  TOOL_INPUT_INVALID: 422,
  TOOL_OUTPUT_INVALID: 502,
  TOOL_EXECUTION_DENIED: 403,
  NETWORK_POLICY_DENIED: 403,
  HOST_NOT_ALLOWED: 403,
  PROTOCOL_NOT_ALLOWED: 403,
  PRIVATE_NETWORK_DENIED: 403,
  SSRF_BLOCKED: 403,
  REDIRECT_DENIED: 403,
  REQUEST_TOO_LARGE: 413,
  RESPONSE_TOO_LARGE: 502,
  EXTERNAL_TIMEOUT: 504,
  EXTERNAL_RATE_LIMITED: 429,
  EXTERNAL_PROVIDER_ERROR: 502,
  EXTERNAL_RESULT_UNKNOWN: 502,
  WEBHOOK_SIGNATURE_INVALID: 401,
  WEBHOOK_TIMESTAMP_INVALID: 401,
  WEBHOOK_REPLAYED: 409,
  WEBHOOK_EVENT_NOT_ALLOWED: 422,
  WEBHOOK_ENDPOINT_REVOKED: 404,
  WEBHOOK_ENDPOINT_SUSPENDED: 404,
  WEBHOOK_TRIGGER_DENIED: 403,
  WEBHOOK_DELIVERY_CONFLICT: 409,
  // Runtime de agentes e modelos (ARDEN-BE-007).
  AGENT_NOT_FOUND: 404,
  AGENT_NOT_ACTIVE: 409,
  AGENT_SUSPENDED: 409,
  AGENT_REVOKED: 409,
  AGENT_VERSION_NOT_FOUND: 404,
  AGENT_VERSION_NOT_PUBLISHED: 409,
  AGENT_VERSION_RETIRED: 409,
  MODEL_PROVIDER_NOT_AVAILABLE: 404,
  MODEL_PROVIDER_DISABLED: 409,
  MODEL_CONFIGURATION_NOT_FOUND: 404,
  MODEL_CONFIGURATION_NOT_ACTIVE: 409,
  AGENT_CONTEXT_TOO_LARGE: 413,
  AGENT_INPUT_INVALID: 422,
  AGENT_OUTPUT_INVALID: 502,
  AGENT_OUTPUT_REPAIR_EXHAUSTED: 502,
  AGENT_TOOL_NOT_ALLOWED: 403,
  AGENT_TOOL_CALL_INVALID: 422,
  AGENT_TOOL_CALL_LIMIT_EXCEEDED: 409,
  AGENT_TOOL_REQUIRES_APPROVAL: 403,
  AGENT_TURN_LIMIT_EXCEEDED: 409,
  AGENT_TOKEN_LIMIT_EXCEEDED: 409,
  AGENT_COST_LIMIT_EXCEEDED: 409,
  AGENT_TIMEOUT: 504,
  MODEL_RATE_LIMITED: 429,
  MODEL_PROVIDER_ERROR: 502,
  MODEL_RESULT_UNKNOWN: 502,
  MODEL_CONTENT_FILTERED: 422,
  AGENT_EVALUATION_FAILED: 409,
  AGENT_PROMPT_INJECTION_DETECTED: 403,
  AGENT_CONTEXT_SOURCE_NOT_ALLOWED: 403,
  AGENT_CONTEXT_SOURCE_NOT_FOUND: 404,
  AGENT_CONTEXT_SOURCE_INVALID: 422,
  AGENT_CONTEXT_BUDGET_EXCEEDED: 413,
  AGENT_RESULT_NOT_FOUND: 404,
  AGENT_GOVERNANCE_BLOCKED: 409,
  AGENT_USAGE_INVALID: 422,
  RATE_LIMITED: 429,
  DEPENDENCY_UNAVAILABLE: 503,
  INTERNAL_ERROR: 500,
};
