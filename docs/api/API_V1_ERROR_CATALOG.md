...
## ARDEN-BE-007 — Runtime de agentes e modelos

Códigos SEMANTICAMENTE NOVOS (contratados em ARDEN-BE-007.1). Nenhuma mensagem expõe
prompt, contexto, resposta bruta do modelo ou segredo. Persistência e runtime chegam em
fases futuras — aqui é só o contrato.

| Código | HTTP | exposição pública | retryable | descrição |
| --- | --- | --- | --- | --- |
| `AGENT_NOT_FOUND` | 404 | sim | não | agente inexistente no tenant |
| `AGENT_NOT_ACTIVE` | 409 | sim | não | agente fora de estado utilizável |
| `AGENT_SUSPENDED` | 409 | sim | não | agente suspenso |
| `AGENT_REVOKED` | 409 | sim | não | agente revogado (terminal) |
| `AGENT_VERSION_NOT_FOUND` | 404 | sim | não | versão inexistente |
| `AGENT_VERSION_NOT_PUBLISHED` | 409 | sim | não | etapa exige versão publicada |
| `AGENT_VERSION_RETIRED` | 409 | sim | não | versão retirada não executa |
| `MODEL_PROVIDER_NOT_AVAILABLE` | 404 | sim | não | provider desconhecido |
| `MODEL_PROVIDER_DISABLED` | 409 | sim | não | provider desabilitado/produção proibida |
| `MODEL_CONFIGURATION_NOT_FOUND` | 404 | sim | não | configuração de modelo inexistente |
| `MODEL_CONFIGURATION_NOT_ACTIVE` | 409 | sim | não | configuração inativa/revogada |
| `AGENT_CONTEXT_TOO_LARGE` | 413 | sim | não | contexto excede o limite da política |
| `AGENT_INPUT_INVALID` | 422 | sim | não | input não casa com `inputSchema` |
| `AGENT_OUTPUT_INVALID` | 502 | sim (sanitizado) | não | saída do modelo não valida contra `outputSchema` |
| `AGENT_OUTPUT_REPAIR_EXHAUSTED` | 502 | sim | não | tentativas de correção esgotadas |
| `AGENT_TOOL_NOT_ALLOWED` | 403 | sim | não | alias fora da allowlist da versão |
| `AGENT_TOOL_CALL_INVALID` | 422 | sim | não | tool call malformada |
| `AGENT_TOOL_CALL_LIMIT_EXCEEDED` | 409 | sim | não | excedeu limite de tool calls |
| `AGENT_TOOL_REQUIRES_APPROVAL` | 403 | sim | não | tool call exige autorização humana (BE-004) |
| `AGENT_TURN_LIMIT_EXCEEDED` | 409 | sim | não | excedeu `maximumTurns` |
| `AGENT_TOKEN_LIMIT_EXCEEDED` | 409 | sim | não | excedeu tetos de tokens |
| `AGENT_COST_LIMIT_EXCEEDED` | 409 | sim | não | excedeu teto de custo estimado |
| `AGENT_TIMEOUT` | 504 | sim | condicional | timeout do agente/etapa |
| `MODEL_RATE_LIMITED` | 429 | sim | condicional (Retry-After) | 429 do provider |
| `MODEL_PROVIDER_ERROR` | 502 | sim (sanitizado) | condicional | erro do provider |
| `MODEL_RESULT_UNKNOWN` | 502 | sim | **não auto** | resultado do modelo indeterminado |
